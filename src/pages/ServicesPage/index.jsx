...
const Check = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0, marginTop: 1 }}>
    <path d="M5 13l4 4L19 7" stroke="#C9A050" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
)
    ...
    <div className="min-h-screen pt-24 pb-20" style={{ background: '#1A1A2E' }}>
      <div className="container">
        ...
        <div className="flex items-center gap-2 text-sm mb-8" style={{ color: 'rgba(232,228,220,0.4)' }}>
          ...
          <span>/</span>
          <span style={{ color: '#C9A050' }}>Дизайн-проект</span>
        </div>
            ...
            return (
              <FadeInView key={t.id} delay={i * 0.07}>
                <div
                  ...
                  style={{
                    background: isPopular ? 'rgba(201,160,80,0.07)' : '#1E2240',
                    border: isPopular ? '1px solid #C9A050' : '1px solid rgba(201,160,80,0.15)',
                    boxShadow: isPopular ? '0 0 40px rgba(201,160,80,0.1)' : undefined,
                  }}
                >
                  {isPopular && (
                    <div className="absolute -top-3 left-6">
                      <span className="text-[10px] font-bold uppercase tracking-[0.12em] px-3 py-1 rounded-full"
                        style={{ background: '#C9A050', color: '#1A1A2E' }}>
                        Популярный
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <span className="text-4xl font-black" style={{ color: 'rgba(201,160,80,0.25)' }}>{t.num}</span>
                    {t.includes3D && (
                      <span className="text-[10px] font-semibold px-2.5 py-1 rounded-full"
                        style={{ color: '#C9A050', background: 'rgba(201,160,80,0.1)', border: '1px solid rgba(201,160,80,0.3)' }}>
                        3D включено
                      </span>
                    )}
                  </div>
                  ...
                  <div className="text-2xl font-black mb-5" style={{ color: '#C9A050' }}>{t.priceLabel}</div>
                  ...
                  <Link to="/contacts"
                    ...
                    style={isPopular
                      ? { background: '#C9A050', color: '#1A1A2E', borderColor: '#C9A050' }
                      : { background: 'transparent', color: '#C9A050', borderColor: 'rgba(201,160,80,0.45)' }
                    }
                    onMouseEnter={(e) => {
                      if (isPopular) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#C9A050' }
                      else { e.currentTarget.style.background = 'rgba(201,160,80,0.08)'; e.currentTarget.style.borderColor = '#C9A050' }
                    }}
                    onMouseLeave={(e) => {
                      if (isPopular) { e.currentTarget.style.background = '#C9A050'; e.currentTarget.style.color = '#1A1A2E' }
                      else { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'rgba(201,160,80,0.45)' }
                    }}
                  >
                    {t.btnText}
                  </Link>
          ...
          {extras.map((ex, i) => (
            <FadeInView key={ex.id} delay={i * 0.07}>
              <div className="rounded-2xl p-6 flex flex-col h-full transition-all duration-300 hover:-translate-y-0.5"
                style={{ background: '#1E2240', border: '1px solid rgba(201,160,80,0.12)' }}
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'rgba(201,160,80,0.35)' }}
                onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'rgba(201,160,80,0.12)' }}
              >
                ...
                <div className="font-bold mb-4" style={{ color: '#C9A050' }}>{ex.priceLabel}</div>
        ...
        <FadeInView>
          <div className="rounded-3xl p-10 text-center max-w-xl mx-auto"
            style={{ background: '#1E2240', border: '1px solid rgba(201,160,80,0.2)' }}>
