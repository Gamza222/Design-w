        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((step, i) => (
            <FadeInView key={step.num} delay={i * 0.1}>
              <div className="relative bg-[#1E2240] border border-[rgba(201,169,122,0.12)] rounded-2xl p-6 h-full hover:border-[rgba(201,169,122,0.3)] transition-all duration-300 group">
                {/* Соединительная линия */}
                {i < steps.length - 1 && (
                  <div className="hidden lg:block absolute top-10 left-full w-6 h-px bg-gradient-to-r from-[rgba(201,169,122,0.3)] to-transparent z-10" />
                )}
                <div className="text-3xl mb-3">{step.icon}</div>
                <span className="text-4xl font-bold text-[rgba(201,169,122,0.15)] block mb-2">{step.num}</span>
